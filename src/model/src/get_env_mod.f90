...
! --------------------------------------------------------------------------------
        subroutine get_env_int (env_value, env_var, default_env_value, logdev)
          ...
          !change the if.esle here
          if ( trim(env_var) == 'N_EMIS_GR') then
             env_value = 1
             regular = .true.
          else if ( trim(env_var) == 'N_EMIS_PT') then
             env_value = 2
             regular = .true.
          else
...
! --------------------------------------------------------------------------------
        subroutine get_env_char (env_value, env_var, default_env_value, logdev)
          ...
          integer :: loc_logdev, length, STATUS !add STATUS for ENVSTR in aqm_methods
          logical :: default, regular
          character (50) :: myfmt
          ...
          !change the if.esle here
          if ( trim(env_var) == 'MISC_CTRL_NML') then
             call nameval ('MISC_CTRL', env_value)
             regular = .true.
          else if ( trim(env_var) == 'DESID_CTRL_NML') then
             call nameval ('DESID_CTRL', env_value)
             regular = .true.
          else if ( trim(env_var) == 'DESID_CHEM_CTRL_NML') then
             call nameval ('DESID_CHEM_CTRL', env_value)
             regular = .true.
          else if ( trim(env_var) == 'gc_matrix_nml') then
             call nameval ('gc_matrix_nml', env_value)
             regular = .true.
          else if ( trim(env_var) == 'ae_matrix_nml') then
             call nameval ('ae_matrix_nml', env_value)
             regular = .true.
          else if ( trim(env_var) == 'nr_matrix_nml') then
             call nameval ('nr_matrix_nml', env_value)
             regular = .true.
          else if ( trim(env_var) == 'tr_matrix_nml') then
             call nameval ('tr_matrix_nml', env_value)
             regular = .true.
          else if ( trim(env_var) == 'GR_EMIS_LAB_001') then
             env_value = 'GRIDDED_EMIS'
             regular = .true.
          else if ( trim(env_var) == 'STK_EMIS_LAB_001') then
             env_value = 'PT_FIRES'
             regular = .true.
          else if ( trim(env_var) == 'STK_EMIS_LAB_002') then
             env_value = 'PT_OTHER'
             regular = .true.
          else if ( trim(env_var) == 'GRID_NAME') then
             !add spaces for 'GRID_NAME =STRTEMP(1:16)' in RUNTIME_VAR
             env_value = 'Cubed-Sphere    '
             regular = .true.
          else if ( trim(env_var) == 'BIOG_SPRO') then
             CALL ENVSTR('BIOG_SPRO', 'Speciation profile for biogenics', &
                         default_env_value, env_value, STATUS)
             regular = .true.
          else
             env_value = default_env_value
             default = .true.
...
! --------------------------------------------------------------------------------
        subroutine get_env_logical (env_value, env_var, default_env_value, logdev)
          ...
          integer :: length, status !add status for envyn in aqm_methods
          integer :: loc_logdev
          logical :: default, regular
          ...
          !change the if.esle here
          if (trim(env_var) == 'CTM_EMISCHK') then
             env_value = .FALSE.
             regular = .true.
          else if (trim(env_var) == 'CTM_GRAV_SETL') then
             env_value = ENVYN('CTM_GRAV_SETL','GRAV_SETL setting',        &
                                default_env_value,STATUS)
             regular = .true.
          else if (trim(env_var) == 'CTM_WB_DUST') then
             env_value = ENVYN('CTM_WB_DUST','In-line dust emissions',     &
                                default_env_value,STATUS)
             regular = .true.
